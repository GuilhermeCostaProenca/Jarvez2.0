...
- [-] E12. Expandir a validacao real para `jarvez-mcp-home-assistant`, `jarvez-mcp-thinq` e `jarvez-mcp-rpg`
  Notas: iniciado por `jarvez-mcp-home-assistant`, expandido para `jarvez-mcp-thinq`, `jarvez-mcp-rpg` e agora para um recorte inicial de `ac_*` em cima do server MCP `thinq`; `backend/actions.py` tenta o MCP `home_assistant` primeiro em `call_service`, `turn_light_on`, `turn_light_off` e `set_light_brightness`, tenta o MCP `thinq` primeiro em `thinq_status` e `thinq_list_devices`, tenta o MCP `rpg` primeiro em `rpg_get_knowledge_stats` e `rpg_search_knowledge`, e agora tenta o MCP `thinq` tambem em `ac_get_status`, `ac_turn_on` e `ac_turn_off`, sempre com fallback legacy preservado. Validacoes reais feitas com `dispatch_action("turn_light_on")`, `dispatch_action("thinq_status")`, `dispatch_action("rpg_get_knowledge_stats")`, `dispatch_action("rpg_search_knowledge")` e `dispatch_action("ac_get_status")`; no recorte AC o backend retornou `provider="mcp"`, `fallback_used=false`, `mcp_server="thinq"`, `mcp_tool="thinq_get_device_state"` e erro de configuracao ThinQ no cenario atual, sem mover preferencias ou automacoes locais do Jarvez.
