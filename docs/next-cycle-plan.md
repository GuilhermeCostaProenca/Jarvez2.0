...
- [-] E12. Expandir a validacao real para `jarvez-mcp-home-assistant`, `jarvez-mcp-thinq` e `jarvez-mcp-rpg`
  Notas: iniciado por `jarvez-mcp-home-assistant` e expandido para `jarvez-mcp-thinq`; `backend/actions.py` agora tenta o MCP `home_assistant` primeiro em `call_service`, `turn_light_on`, `turn_light_off` e `set_light_brightness`, e tenta o MCP `thinq` primeiro em `thinq_status` e `thinq_list_devices`, sempre com fallback legacy preservado. Validacoes reais feitas com `dispatch_action("turn_light_on")` e `dispatch_action("thinq_status")`; no ThinQ o backend retornou `provider="mcp"`, `fallback_used=false`, mensagem `ThinQ parcialmente configurado.` e `devices_error="missing THINQ_PAT"` no cenario atual. `rpg` segue pendente.
